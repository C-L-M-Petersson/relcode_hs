...
OUTDIR=outFiles/KRAKEN/run4/PURITY/
echo -n "" > "$OUTDIR"all.res
...
do
    ...
    echo -n "$FWHM " >> "$OUTDIR"all.res
    paste "$OUTDIR"purity1ph.FWHM=$FWHM.res      \
          "$OUTDIR"concurrence1ph.FWHM=$FWHM.res \
          "$OUTDIR"purity2ph.FWHM=$FWHM.res      \
          "$OUTDIR"concurrence2ph.FWHM=$FWHM.res \
            >> "$OUTDIR"all.res
done
